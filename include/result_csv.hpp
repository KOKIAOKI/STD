...
public:
  ResultCsv(const std::string& folder_name, const std::string& gt_pose_file_name) {
    ...
    if (csv_file.is_open()) {
      csv_file << "file_name,exe_time,relative_trans,relative_rot,gt_src_x,gt_src_y,gt_src_z,gt_src_q_x,gt_src_q_y,gt_src_q_z,gt_src_q_w,gt_tar_x,gt_tar_y,gt_tar_z,gt_tar_q_x,gt_"
                  "tar_q_y,gt_tar_q_z,gt_tar_q_w,relative_x,relative_y,relative_z,relative_q_x,relative_q_y,relative_q_z,relative_q_w\n";
    }
  }
  ...
  void write(const std::string& file_name, const double exe_time, const Eigen::Matrix4d& gt_src, const Eigen::Matrix4d& gt_tar, const Eigen::Matrix4d& relative) {
    ...
    double relative_trans = relative.block<3, 1>(0, 3).norm();
    double relative_rot = Eigen::AngleAxisd(relative.block<3, 3>(0, 0)).angle();
    ...
    csv_file << file_name << "," << exe_time << "," << relative_trans << "," << relative_rot << "," << gt_src_x << "," << gt_src_y << "," << gt_src_z << "," << gt_src_q_x << ","
             << gt_src_q_y << "," << gt_src_q_z << "," << gt_src_q_w << "," << gt_tar_x << "," << gt_tar_y << "," << gt_tar_z << "," << gt_tar_q_x << "," << gt_tar_q_y << ","
             << gt_tar_q_z << "," << gt_tar_q_w << "," << relative_x << "," << relative_y << "," << relative_z << "," << relative_q_x << "," << relative_q_y << "," << relative_q_z
             << "," << relative_q_w << "\n";
  }
};
